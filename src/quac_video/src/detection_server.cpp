...
DetectionServer::DetectionServer(const std::string& name, const std::string& topic_name, DetectionCallback callback) : Node(name), tf_buffer(this->get_clock()), tf_listener(tf_buffer), detection_callback(callback)
{
  ...
  for (int i = 0; i < topic_handlers.size(); i++)
  {
    ...
    topic_handlers[i]->object_publisher = create_publisher<quac_interfaces::msg::DetectedObjectArray>(topics[i] + "/" + topic_name, 10);
  }
}
